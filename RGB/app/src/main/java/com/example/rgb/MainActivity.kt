...
class MainActivity : AppCompatActivity() {
    ...
    init {
        this.palet.add(Color("Vermelho", 100, 0, 0))
        this.palet.add(Color("Verde", 0, 100, 0))
        this.palet.add(Color("Azul", 0, 0, 100))
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        this.rvPaleta.adapter = Paletadapter(this.palet.get(), OnItemClick())
        ...
        this.formResult = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            if (it.resultCode == RESULT_OK){
                val cor = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    it.data?.getSerializableExtra("COR", Color::class.java)
                } else {
                    it.data?.getSerializableExtra("COR")
                } as Color

                if (cor.id == this.palet.size()) {
                    this.palet.add(cor)
                    (this.rvPaleta.adapter as Paletadapter).notifyItemInserted(this.palet.size())
                } else {
                    Log.i("COR", "Id: ${cor.id}" + cor.toString())
                    Log.i("PALETA SIZE", this.palet.size().toString())
                    val changeCor = this@MainActivity.palet.get(cor.id)
    ...
    inner class OnItemClick: OnItemClickRecyclerView {
        override fun onItemClick(position: Int) {
            val intent = Intent(this@MainActivity, FormActivity::class.java).apply {
                putExtra("COR", this@MainActivity.palet.get(position))
            }
            this@MainActivity.formResult?.launch(intent)
        }
    }
